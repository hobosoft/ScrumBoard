...
namespace KtSoft.ScrumControls.Renderer
{
	...
	internal class NonVisualStyleWrapper : IRenderer
	{
		...
		public void DrawBox(Graphics g, Font font, Rectangle bounds, string text, TextAlignment textAlignment, bool drawBackground, BorderInfo borders, bool bold, Brush backgroundOverride)
		{
			...
			if (!string.IsNullOrEmpty(text))
			{
				...
				if (bold)
				{
					...
					if (textAlignment == TextAlignment.Left)
						g.DrawString(text, boldFont, foreColourBrush, rcText, StringFormats.Left);
					else if (textAlignment == TextAlignment.Right)
						g.DrawString(text, boldFont, foreColourBrush, rcText, StringFormats.Right);
					else
						g.DrawString(text, boldFont, foreColourBrush, rcText, StringFormats.Center);
				}
				else
				{
					if (textAlignment == TextAlignment.Left)
						g.DrawString(text, font, foreColourBrush, rcText, StringFormats.Left);
					else if (textAlignment == TextAlignment.Right)
						g.DrawString(text, font, foreColourBrush, rcText, StringFormats.Right);
					else
						g.DrawString(text, font, foreColourBrush, rcText, StringFormats.Center);
				}
			}
		}
